...
const useStockTransactionRowActions = (data: StockTransactionType) => {
  ...
  const onDelete = async () => {
    ...
    if (deleteRes.response) {
      const res = await apiQuery.stockTransaction.delete(data.id);
      if (res.success) {
...
export const TableAction = ({ data }: { data: StockTransactionType }) => {
  const isMobile = useIsMobile();
  const { onView, onUpdate, onDelete } = useStockTransactionRowActions(data);
...
export const TableActionContextMenu = ({ data }: { data: StockTransactionType }) => {
  const { onView, onUpdate, onDelete } = useStockTransactionRowActions(data);
  const { data: currentUserRes } = apiQuery.auth.useGetCurrentUser();
  const isSUPERUSER = currentUserRes?.data?.role === "SUPERUSER";
...
export const TableRowsSelect = ({
  ...
  const { isRowSelect, toggleRowSelect, selectedRows } = useTableRowsSelect("stockTransaction");
  const { data: currentUserRes } = apiQuery.auth.useGetCurrentUser();
  const isSUPERUSER = currentUserRes?.data?.role === "SUPERUSER";
...
export const TableSelectAction = () => {
  const { selectedRows, clearRowSelect } = useTableRowsSelect("stockTransaction");
  ...
  const onDelete = async () => {
    ...
    try {
      const res = await apiQuery.stockTransaction.deleteMany(selectedRows);
      clearRowSelect();
