...
const useCountryRowActions = (data: CountryType) => {
  const navigate = useNavigate({ from: "/app/country" })
  ...
  const onDelete = async () => {
    ...
    if (deleteRes.response) {
      const res = await apiQuery.country.delete(data.id)
      if (res.success) {
...
export const TableAction = ({ data }: { data: CountryType }) => {
  const isMobile = useIsMobile()
  const { onView, onUpdate, onDelete } = useCountryRowActions(data)
...
export const TableActionContextMenu = ({ data }: { data: CountryType }) => {
  const { onView, onUpdate, onDelete } = useCountryRowActions(data)
  const { data: currentUserRes } = apiQuery.auth.useGetCurrentUser()
  const isSUPERUSER = currentUserRes?.data?.role === "SUPERUSER"
...
export const TableSelectAction = () => {
  const { selectedRows, clearRowSelect } = useTableRowsSelect("country")
  ...
  const onDelete = async () => {
    ...
    try {
      const res = await apiQuery.country.deleteMany(selectedRows)
      clearRowSelect()
