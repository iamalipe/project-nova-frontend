...
export default function WarehouseDialogAll({ state }: { state: DialogStateType }) {
  const warehouseQuery = apiQuery.warehouse.useGetAll({ page: 0 });
  const { isLoading } = useQueryLoadingState([warehouseQuery]);

  if (isLoading) return <DialogSkeleton />;

  return <DialogMain state={state} data={warehouseQuery?.data?.data} />;
}

const DialogMain = ({ data }: { state: DialogStateType; data?: WarehouseType[] }) => {
    ...
    <Dialog open={true} onOpenChange={(open: boolean) => !open && onClose()}>
      <DialogContent className="flex flex-col sm:max-h-[80vh] sm:max-w-[1200px]">
        <DialogHeader className="flex-none">
          <DialogTitle>All Warehouses</DialogTitle>
          <DialogDescription>Audit and view all warehouses in the system.</DialogDescription>
        </DialogHeader>
