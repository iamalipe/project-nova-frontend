import type { ApiSellGetAll } from "@/api/sell-api";
...
const SELL_ROUTE_FROM = "/app/sell";
const SELL_DIALOG = "Sell";

export type ActionControlsProps<T> = {
  dataTable: DataTableType<T>;
  rawQuery: UseQueryResult<ApiSellGetAll, ApiNormalResponse | Error>;
};
...
const ActionControls = <T,>(props: ActionControlsProps<T>) => {
  ...
  const navigate = useNavigate({ from: SELL_ROUTE_FROM });
  const searchParam = useSearch({
    from: SELL_ROUTE_FROM,
  });
  ...
  return (
    <div className="flex flex-none justify-between">
      ...
      <div className="flex gap-2">
        <Button title="All Sales" variant="outline" onClick={onAllSales}>
          All Sales
        </Button>
        <Button
          title="Create New"
