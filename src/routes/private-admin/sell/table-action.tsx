...
const useSellRowActions = (data: SellType) => {
  ...
  const onDelete = async () => {
    ...
    if (deleteRes.response) {
      const res = await apiQuery.sell.delete(data.id);
      if (res.success) {
...
export const TableAction = ({ data }: { data: SellType }) => {
  const isMobile = useIsMobile();
  const { onView, onUpdate, onDelete } = useSellRowActions(data);
...
export const TableActionContextMenu = ({ data }: { data: SellType }) => {
  const { onView, onUpdate, onDelete } = useSellRowActions(data);
  const { data: currentUserRes } = apiQuery.auth.useGetCurrentUser();
  const isSUPERUSER = currentUserRes?.data?.role === "SUPERUSER";
...
export const TableRowsSelect = ({
  ...
  const { isRowSelect, toggleRowSelect, selectedRows } = useTableRowsSelect("sell");
  const { data: currentUserRes } = apiQuery.auth.useGetCurrentUser();
  const isSUPERUSER = currentUserRes?.data?.role === "SUPERUSER";
...
export const TableSelectAction = () => {
  const { selectedRows, clearRowSelect } = useTableRowsSelect("sell");
  ...
  const onDelete = async () => {
    ...
    try {
      const res = await apiQuery.sell.deleteMany(selectedRows);
      clearRowSelect();
