...
export default function SellDialogAll({ state }: { state: DialogStateType }) {
  const sellQuery = apiQuery.sell.useGetAll({ page: 0 });
  const { isLoading } = useQueryLoadingState([sellQuery]);

  if (isLoading) return <DialogSkeleton />;

  return <DialogMain state={state} data={sellQuery?.data?.data} />;
}

const DialogMain = ({ data }: { state: DialogStateType; data?: SellType[] }) => {
    ...
    <Dialog open={true} onOpenChange={(open: boolean) => !open && onClose()}>
      <DialogContent className="flex flex-col sm:max-h-[80vh] sm:max-w-[1200px]">
        <DialogHeader className="flex-none">
          <DialogTitle>All Sales Transactions</DialogTitle>
          <DialogDescription>Audit and view all POS records in the system.</DialogDescription>
        </DialogHeader>
