...
const useCategoryRowActions = (data: CategoryType) => {
  const navigate = useNavigate({ from: "/app/category" })
  ...
  const onDelete = async () => {
    ...
    if (deleteRes.response) {
      const res = await apiQuery.category.delete(data.id)
      if (res.success) {
...
export const TableAction = ({ data }: { data: CategoryType }) => {
...
export const TableSelectAction = () => {
  const { selectedRows, clearRowSelect } = useTableRowsSelect("category")
  ...
  const onDelete = async () => {
    ...
    try {
      const res = await apiQuery.category.deleteMany(selectedRows)
      clearRowSelect()
