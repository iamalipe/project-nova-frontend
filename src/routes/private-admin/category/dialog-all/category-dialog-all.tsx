...
export type CategoryDialogAllProps = {
  state: DialogStateType
  data?: CategoryType[]
}

const CategoryDialogAll = ({ state }: CategoryDialogAllProps) => {
  const categoryQuery = apiQuery.category.useGetAll({ page: 1, limit: 100 })
  const { isLoading } = useQueryLoadingState([categoryQuery])
...
export default CategoryDialogAll

const DialogMain = ({ data }: CategoryDialogAllProps) => {
    ...
    >
      <DialogContent className="flex flex-col sm:max-h-[80vh] sm:max-w-[1200px]">
        <DialogHeader className="flex-none">
          ...
          <DialogDescription>
            Audit and view all categories in the system.
          </DialogDescription>
        </DialogHeader>
