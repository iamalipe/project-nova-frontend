...
export default function StoreDialogAll({ state }: { state: DialogStateType }) {
  const storeQuery = apiQuery.store.useGetAll({ page: 0 });
  const { isLoading } = useQueryLoadingState([storeQuery]);

  if (isLoading) return <DialogSkeleton />;

  return <DialogMain state={state} data={storeQuery?.data?.data} />;
}

const DialogMain = ({ data }: { state: DialogStateType; data?: StoreType[] }) => {
    ...
    <Dialog open={true} onOpenChange={(open: boolean) => !open && onClose()}>
      <DialogContent className="flex flex-col sm:max-h-[80vh] sm:max-w-[1200px]">
        <DialogHeader className="flex-none">
          <DialogTitle>All Stores</DialogTitle>
          <DialogDescription>Audit and view all stores in the system.</DialogDescription>
        </DialogHeader>
