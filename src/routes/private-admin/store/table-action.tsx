...
const useStoreRowActions = (data: StoreType) => {
  ...
  const onDelete = async () => {
    ...
    if (deleteRes.response) {
      const res = await apiQuery.store.delete(data.id);
      if (res.success) {
...
export const TableAction = ({ data }: { data: StoreType }) => {
  const isMobile = useIsMobile();
  const { onView, onUpdate, onDelete } = useStoreRowActions(data);
...
export const TableActionContextMenu = ({ data }: { data: StoreType }) => {
  const { onView, onUpdate, onDelete } = useStoreRowActions(data);
  const { data: currentUserRes } = apiQuery.auth.useGetCurrentUser();
  const isSUPERUSER = currentUserRes?.data?.role === "SUPERUSER";
...
export const TableRowsSelect = ({
  ...
  const { isRowSelect, toggleRowSelect, selectedRows } = useTableRowsSelect("store");
  const { data: currentUserRes } = apiQuery.auth.useGetCurrentUser();
  const isSUPERUSER = currentUserRes?.data?.role === "SUPERUSER";
...
export const TableSelectAction = () => {
  const { selectedRows, clearRowSelect } = useTableRowsSelect("store");
  ...
  const onDelete = async () => {
    ...
    try {
      const res = await apiQuery.store.deleteMany(selectedRows);
      clearRowSelect();
