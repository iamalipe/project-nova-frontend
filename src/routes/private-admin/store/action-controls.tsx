...
const ActionControls = <T,>(props: ActionControlsProps<T>) => {
  ...
  const navigate = useNavigate({ from: STORE_ROUTE_FROM });
  const searchParam = useSearch({
    from: STORE_ROUTE_FROM,
  });
  ...
  return (
    <div className="flex flex-none justify-between">
      ...
      <div className="flex gap-2">
        <Button title="All Stores" variant="outline" onClick={onAllStores}>
          All Stores
        </Button>
        {isSUPERUSER && (
          <>
