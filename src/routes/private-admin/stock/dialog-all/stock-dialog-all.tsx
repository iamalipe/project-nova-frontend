...
export default function StockDialogAll({ state }: { state: DialogStateType }) {
  const stockQuery = apiQuery.stock.useGetAll({ page: 0 });
  const { isLoading } = useQueryLoadingState([stockQuery]);

  if (isLoading) return <DialogSkeleton />;

  return <DialogMain state={state} data={stockQuery?.data?.data} />;
}

const DialogMain = ({ data }: { state: DialogStateType; data?: StockType[] }) => {
    ...
    <Dialog open={true} onOpenChange={(open: boolean) => !open && onClose()}>
      <DialogContent className="flex flex-col sm:max-h-[80vh] sm:max-w-[1200px]">
        <DialogHeader className="flex-none">
          <DialogTitle>All Stock Items</DialogTitle>
          <DialogDescription>Audit and view all inventory records in the system.</DialogDescription>
        </DialogHeader>
