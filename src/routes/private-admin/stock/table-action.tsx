...
const useStockRowActions = (data: StockType) => {
  ...
  const onDelete = async () => {
    ...
    if (deleteRes.response) {
      const res = await apiQuery.stock.delete(data.id);
      if (res.success) {
...
export const TableAction = ({ data }: { data: StockType }) => {
  const isMobile = useIsMobile();
  const { onView, onUpdate, onDelete } = useStockRowActions(data);
...
export const TableActionContextMenu = ({ data }: { data: StockType }) => {
  const { onView, onUpdate, onDelete } = useStockRowActions(data);
  const { data: currentUserRes } = apiQuery.auth.useGetCurrentUser();
  const isSUPERUSER = currentUserRes?.data?.role === "SUPERUSER";
...
export const TableRowsSelect = ({
  ...
  const { isRowSelect, toggleRowSelect, selectedRows } = useTableRowsSelect("stock");
  const { data: currentUserRes } = apiQuery.auth.useGetCurrentUser();
  const isSUPERUSER = currentUserRes?.data?.role === "SUPERUSER";
...
export const TableSelectAction = () => {
  const { selectedRows, clearRowSelect } = useTableRowsSelect("stock");
  ...
  const onDelete = async () => {
    ...
    try {
      const res = await apiQuery.stock.deleteMany(selectedRows);
      clearRowSelect();
