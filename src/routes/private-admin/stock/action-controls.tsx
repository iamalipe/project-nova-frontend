import type { ApiStockGetAll } from "@/api/stock-api";
...
const STOCK_ROUTE_FROM = "/app/stock";
const STOCK_DIALOG = "Stock";

export type ActionControlsProps<T> = {
  dataTable: DataTableType<T>;
  rawQuery: UseQueryResult<ApiStockGetAll, ApiNormalResponse | Error>;
};
...
const ActionControls = <T,>(props: ActionControlsProps<T>) => {
  ...
  const navigate = useNavigate({ from: STOCK_ROUTE_FROM });
  const searchParam = useSearch({
    from: STOCK_ROUTE_FROM,
  });
  ...
  return (
    <div className="flex flex-none justify-between">
      ...
      <div className="flex gap-2">
        <Button title="All Stock Items" variant="outline" onClick={onAllStocks}>
          All Stock Items
        </Button>
        <Button
          title="Create New"
