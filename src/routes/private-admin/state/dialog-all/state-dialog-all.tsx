...
export type StateDialogAllProps = {
  state: DialogStateType
  data?: StateType[]
}

const StateDialogAll = ({ state }: StateDialogAllProps) => {
  const stateQuery = apiQuery.state.useGetAll({ page: 1, limit: 100 })
  const { isLoading } = useQueryLoadingState([stateQuery])

  if (isLoading) return <DialogSkeleton />

  return <DialogMain state={state} data={stateQuery?.data?.data} />
}

export default StateDialogAll
...
const DialogMain = ({ data }: StateDialogAllProps) => {
    ...
    >
      <DialogContent className="flex flex-col sm:max-h-[80vh] sm:max-w-[1200px]">
        <DialogHeader className="flex-none">
          <DialogTitle>All States</DialogTitle>
          <DialogDescription>
            Audit and view all states in the system.
          </DialogDescription>
        </DialogHeader>
