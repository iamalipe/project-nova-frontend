...
const useStateRowActions = (data: StateType) => {
  const navigate = useNavigate({ from: "/app/state" })
  ...
  const onDelete = async () => {
    ...
    if (deleteRes.response) {
      const res = await apiQuery.state.delete(data.id)
      if (res.success) {
...
export const TableAction = ({ data }: { data: StateType }) => {
  const isMobile = useIsMobile()
  const { onView, onUpdate, onDelete } = useStateRowActions(data)
...
export const TableActionContextMenu = ({ data }: { data: StateType }) => {
  const { onView, onUpdate, onDelete } = useStateRowActions(data)
  const { data: currentUserRes } = apiQuery.auth.useGetCurrentUser()
  const isSUPERUSER = currentUserRes?.data?.role === "SUPERUSER"
...
export const TableSelectAction = () => {
  const { selectedRows, clearRowSelect } = useTableRowsSelect("state")
  ...
  const onDelete = async () => {
    ...
    try {
      const res = await apiQuery.state.deleteMany(selectedRows)
      clearRowSelect()
