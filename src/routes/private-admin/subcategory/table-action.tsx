...
const useSubcategoryRowActions = (data: SubcategoryType) => {
  const navigate = useNavigate({ from: "/app/subcategory" })
  ...
  const onDelete = async () => {
    ...
    if (deleteRes.response) {
      const res = await apiQuery.subcategory.delete(data.id)
      if (res.success) {
...
export const TableAction = ({ data }: { data: SubcategoryType }) => {
  const isMobile = useIsMobile()
  const { onView, onUpdate, onDelete } = useSubcategoryRowActions(data)
...
export const TableActionContextMenu = ({ data }: { data: SubcategoryType }) => {
  const { onView, onUpdate, onDelete } = useSubcategoryRowActions(data)
  const { data: currentUserRes } = apiQuery.auth.useGetCurrentUser()
  const isSUPERUSER = currentUserRes?.data?.role === "SUPERUSER"
...
export const TableRowsSelect = ({
  ...
  const { isRowSelect, toggleRowSelect, selectedRows } =
    useTableRowsSelect("subcategory")
  const { data: currentUserRes } = apiQuery.auth.useGetCurrentUser()
  const isSUPERUSER = currentUserRes?.data?.role === "SUPERUSER"
...
export const TableSelectAction = () => {
  const { selectedRows, clearRowSelect } = useTableRowsSelect("subcategory")
  ...
  const onDelete = async () => {
    ...
    try {
      const res = await apiQuery.subcategory.deleteMany(selectedRows)
      clearRowSelect()
