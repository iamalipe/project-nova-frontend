import type { ApiSubcategoryGetAll } from "@/api/subcategory-api";
...
type SubcategoryProps = {
  rawQuery: UseQueryResult<ApiSubcategoryGetAll, ApiNormalResponse | Error>;
};

const Subcategory = ({ rawQuery }: SubcategoryProps) => {
  const queryData = rawQuery.data;
  if (!queryData) throw Error("Something wrong");

  const tableVisibility = useTableVisibility("subcategory");

  const pagination = usePagination({
    initialPageSize: queryData.pagination.limit,
    initialPageIndex: queryData.pagination.page,
    routeFrom: SUBCATEGORY_ROUTE_FROM,
  });
